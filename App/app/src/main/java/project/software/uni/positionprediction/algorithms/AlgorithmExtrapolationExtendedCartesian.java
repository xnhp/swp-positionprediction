...
import project.software.uni.positionprediction.datatypes.Vector3;
import project.software.uni.positionprediction.util.Message;

public class AlgorithmExtrapolationExtendedCartesian extends PredictionAlgorithmReturnsTrajectory {
    ...
    // Constructor
    public AlgorithmExtrapolationExtendedCartesian(Context c) {
        super(c);
    }
    ...
    private PredictionResultData make_prediction(Trajectory data, Date date_past, Date date_pred) {
        ...
        // 1. Get old vectors
        ArrayList<Vector3> vector_collection = get_past_vectors(data, date_past);
     ...
     * @return
     */
    private Location predict_next_Location(Trajectory data, ArrayList<Vector3> vector_collection, double pred_factor){
        if (vector_collection == null) {
            return null;
        }

        // avg has altitude or not based on the data that is passed in
        Vector3 avg = weighted_average(vector_collection);
        // curr_loc has altitude or not based on the data that is passed in
        Vector3 curr_loc = Vector3.fromLocation(data.getLocation(data.size() - 1).to3D());

        if (avg == null) {
            return null;
        }

        return Vector3.add(curr_loc, Vector3.multiply(avg, pred_factor)).toLocation();
    }
     ...
     * @return
     */
    private ArrayList<Vector3> get_past_vectors(Trajectory data, Date date_past) {

        ArrayList<Vector3> vector_collection = new ArrayList<Vector3>();
        int n = data.size() - 1;
        int c = 0;
        ...
        for (int t = 1; t <= n; t++) {
            ...
            // Get n-th point
            Vector3 vec_n = Vector3.fromLocation(data.getLocation(n));

            // Get n-t-th point
            Vector3 vec_old = Vector3.fromLocation(data.getLocation(n - t));

            // Compute vector between them
            Vector3 vec_delta = Vector3.sub(vec_n, vec_old);

            // Compute average
            Vector3 vec_avg = Vector3.devide(vec_delta, t);
     ...
     * @return
     */
    private Vector3 weighted_average(ArrayList<Vector3> collection) {
        ...
        for (int i = 0; i < collection.size(); i++) {
            ...
            sum = Vector3.add(sum, collection.get(i));
        }
